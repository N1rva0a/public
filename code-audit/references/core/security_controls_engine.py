...
class CodeScanner:
    ...
    def __init__(self, adapter: Dict, operations: Dict[str, SensitiveOperation], exclude_tests: bool = False):
    ...
    def scan_directory(self, path: str) -> List[OperationContext]:
        ...
        for file_path in path.rglob('*'):
            ...
            # 跳过常见的非代码目录。tests 由调用方显式决定是否排除。
            skip_dirs = ['vendor', 'node_modules', '.git', 'dist', 'build', '__pycache__']
            if self.exclude_tests:
                skip_dirs.extend(['test', 'tests'])
...
class SecurityControlsEngine:
    ...
    def audit(self, project_path: str, language: str) -> List[Finding]:
        ...
        # 扫描代码
        scanner = CodeScanner(adapter, operations, exclude_tests=args.exclude_tests)
...
支持的语言: php, java, python, go, javascript
    ...
    parser.add_argument('--exclude-tests', action='store_true',
                        help='显式排除 test/ 和 tests/ 目录；默认保留并交由调用方决定覆盖范围')
