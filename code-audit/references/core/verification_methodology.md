...
- `SKILL.md` 的 E1-E6 证据槽和 lifecycle 状态机才是正式裁决来源。
- 本文件中的置信度分数仅可作为验证计划辅助，不得替代 `CONFIRMED` / `PROBABLE` / `HYPOTHESIS` 的状态判断。
- 缺少动态验证不等于可以静默丢弃代码证据明确的路径；此时应保留在 `PROBABLE` 或 `HYPOTHESIS`。
